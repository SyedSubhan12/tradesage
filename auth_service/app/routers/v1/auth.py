    ...
    try:
        ...
        if not authorization or not authorization.startswith("Bearer "):
            await log_audit_event(
                ...
                details={
                    "client_ip": request.client.host,
                    "reason": "missing_or_malformed_refresh_token_header"
                }
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        refresh_token_raw = authorization.split(" ")[1].strip()
        if not refresh_token_raw:
            await log_audit_event(
                event_type="token_refresh_failed",
                user_id=None,
        ...
        try:
            ...
            await log_audit_event(
                event_type="token_refresh_failed",
                user_id=token_payload.get("user_id") if 'token_payload' in locals() and token_payload else None,
                details={
                    "client_ip": request.client.host,
        ...
        return TokenResponse(
            ...
            refresh_token=refresh_token_raw, # Return the same refresh token
            token_type="bearer",
            expires_in=int(access_token_expires.total_seconds()),
            tenant_status=tenant.status.value
        )
