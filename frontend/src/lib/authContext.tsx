...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleLogout = useCallback(async (showToast = true) => {
    try {
      let logoutResult: { refresh_token_deletion_result?: boolean; refresh_token_deletion_reason?: string; logout?: boolean; authenticated?: boolean; } = { logout: true };
      if (state.token) {
        logoutResult = await api.logout();
      }
      // If token deletion failed, log and optionally show toast
      if (logoutResult && logoutResult.refresh_token_deletion_result === false) {
        const reason = logoutResult.refresh_token_deletion_reason || 'unknown';
        console.warn('Refresh token deletion not successful during logout:', reason);
        if (showToast) {
          toast.info(`Logged out, but refresh token deletion was not successful: ${reason}`);
        }
      } else if (showToast) {
